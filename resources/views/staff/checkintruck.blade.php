...
@section('staff-content')
    ...
    @else
        <div class="container">
           ...
           {{-- Truck Log --}}
            <div class="row">
                @if ($get_truck_info->id_truck_status >= '2')
                    <div class="col row">
                        ...
                        @if ($get_truck_info->id_truck_status == '4')
                            <div class="col">
                                <button class="btn btn-default btn-primary" type="button" class="btn btn-primary"
                                    data-bs-toggle="modal" data-bs-target="#exampleModal">Xác nhận đến</button>
                                            ...
                                            <form action="/thynx/truck-log" method="GET">
                                                ...
                                                <div class="modal-body">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" hidden name="id_truck"
                                                            value="{{ $get_truck_info->id_truck }}">
                ...
                @else
                    <div class="col">
                        <a href="{{ URL::to('/truck-log?process=roaldout&id_truck=' . $get_truck_info->id_truck . '&id_station=' . $current_station->id_station) }}"
                            class="btn btn-dark "> Khởi hành</a>
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modal_1">Giao gói
                            hàng</button>
                        <a href="{{ URL::to('/truck-log?process=checkout&id_truck=' . $get_truck_info->id_truck . '&id_station=' . $current_station->id_station) }}"
                            class="btn btn-default btn-danger ">Kết thúc phiên</a>
                    </div>
                    <div class="modal fade" id="modal_1" tabindex="-1" aria-labelledby="exampleModalLabel"
                        aria-hidden="true">
                                ...
                                <form action="{{ URL::to('/truck-log') }}" method="get">
                                    ...
                                    <div class="modal-body form-group row">
                                        ...
                                        <input type="text" class="visually-hidden"
                                            value="{{ $get_truck_info->id_truck }}" name="id_truck">
                                        <select name="id_bag" id="id_bag" class="form-select">
                                            @foreach ($truck_bag as $bag)
                                                <option value="{{ $bag->id_bag }}">
                                                    {{ $bag->bag_name . ' từ: ' . $bag->station_name }}</option>
                                            @endforeach
        ...
        @else
            <div class="col">
                ...
                <a href="{{ URL::to('/truck-log?process=checkin&id_truck=' . $get_truck_info->id_truck) }}"
    ...
    <div class="stepper-wrapper">
        @php
            $i = 1;
        @endphp
        @foreach ($get_log_today as $item)
            @if ($item->truck_status == 'Đã nhận')
            <div class="stepper-item completed">
                <div class="step-counter">{{$i}}</div>
                <div class="step-name">{{$item->truck_status}}</div>
            </div>  
            @else
            <div class="stepper-item completed">
                <div class="step-counter">{{$i}}</div>
                <div class="step-name">{{$item->note}}</div>
            </div>
            @endif
            @php
                $i++;
